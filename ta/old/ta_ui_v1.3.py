...
class UI:
    ...
    def _draw_dd_label(self, index, x_start, bar_width):
        ...
        try:
            ...
            if font_large:
                # Calculer position centrée
                text_width = len(text) * 16  # 16 pixels par caractère
                text_x = x_start + (bar_width - text_width) // 2
                text_y = label_y
                
                # Afficher en blanc sur fond noir
                self.tft.text(font_large, text, text_x, text_y, st7789.WHITE, st7789.BLACK)
            else:
    ...
    def status(self, text):
        ...
        try:
            ...
            # Afficher texte avec font si disponible
            if font_small and text:
                # Calculer combien de caractères par ligne (8 pixels par caractère)
                ...
                if len(text) <= max_chars_per_line:
                    ...
                    # Découper intelligemment par mots
                    words = text.split()
                    current_line = ""
                    for word in words:
                ...
                # Afficher jusqu'à 2 lignes (16 pixels de hauteur chacune)
                y_offset = self.y_log + 5
                for i, line in enumerate(lines[:2]):
                    if y_offset + 16 > self.y_log + self.zone_log_height:
                        break
                    self.tft.text(font_small, line, 8, y_offset, st7789.CYAN, st7789.BLACK)
                    y_offset += 18  # 16 pixels de hauteur + 2 pixels d'espacement
                    
            elif text:
                # Fallback: barres colorées
