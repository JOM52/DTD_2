"""
Project: DTD - ta_app.py v2.3.2
Change: expose class TaApp for backward-compat with imports expecting TaApp;
        keep App implementation (v2.3.0), add alias subclass.
"""
...
class App:
    def __init__(self, *args, **kwargs):
        """
        v2.3.2: compatibilité
        - App()                   -> construit UI et Radio par défaut
        - App(ui, radio)          -> utilise instances fournies
        - App(ui=..., radio=...)  -> idem
        """
        ui = None
        radio = None
        if len(args) == 0 and not kwargs:
            try:
                ...
                ui = UI()
                radio = Radio433(__import__("ta_config").ta_config.RADIO)
            except Exception as e:
                # Retente imports explicites
                try:
                    import ta_config as _cfg
                    from ta_ui import UI
                    from ta_radio_433 import Radio433
                    ui = UI()
                    radio = Radio433(_cfg.RADIO)
                except Exception as e2:
                    raise RuntimeError("App(): création auto UI/Radio impossible: {}".format(e2))
        else:
            ui = kwargs.get("ui", None) if kwargs else None
            radio = kwargs.get("radio", None) if kwargs else None
            if len(args) >= 1: ui = args[0]
            if len(args) >= 2: radio = args[1]
            if ui is None or radio is None:
                raise TypeError("App.__init__ attend (ui, radio) ou aucun argument")
        self.ui = ui
        self.radio = radio
...
# Alias/compat pour anciens imports
class TaApp(App):
    pass

logger.info("ta_app.py v2.3.2 chargé (alias TaApp)", "app")
